...
Exception handling while streaming events follows some simple rules
 ...
 - `IOException` and its subclasses are treated as temporary failures. If an `IOException` occurs while opening the initial connection, it is considered a temporary failure. 
Library will automatically recover from this type of failure by retrying processing attempt after time interval specified by the `BackoffStrategy`.
